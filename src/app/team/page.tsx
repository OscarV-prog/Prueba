        ...
        try {
            ...
            if (session?.user?.activeWorkspaceId) {
                ...
                if (wsRes.ok) {
                    ...
                    const active = workspaces.find((w: any) => w.id === session.user.activeWorkspaceId);
                    setIsPersonal(active?.name === "Personal");
                }
            }
        } finally {
            setLoading(false);
        ...
        <div className="min-h-[calc(100vh-64px)] p-6 sm:p-10">
            <div className="mx-auto max-w-7xl space-y-10">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-gray-800 pb-8">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">{t.team.title}</h1>
                        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{t.team.subtitle}</p>
                    </div>
                    {!isPersonal && (
                    ...
                    {isPersonal && (
                        <div className="flex items-center space-x-2 rounded-xl bg-indigo-50 px-4 py-2 text-sm font-medium text-indigo-600 dark:bg-indigo-900/10 dark:text-indigo-400">
                            <Check className="h-4 w-4" />
                            <span>{t.common.personalSpace}</span>
                        </div>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
                    ...
                    <div className="lg:col-span-8 space-y-8">
                        <div className="flex items-center space-x-2 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                            ...
                            <span>{t.team.memberWorkload}</span>
                        </div>
                                    ...
                                    <div className="flex items-center justify-between mb-6">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] font-medium text-gray-500 dark:text-gray-400">{t.team.collaborator}</p>
                                            </div>
                                        </div>
                                        <span className="text-xs font-bold bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 px-3 py-1 rounded-full">
                                            {stats.total} {t.team.tasks}
                                        </span>
                                    </div>

                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between text-xs font-medium">
                                            <span className="text-gray-500 dark:text-gray-400">{t.team.inProgress}</span>
                                        ...
                                        <div className="flex items-center justify-between pt-2">
                                            <div className="flex items-center space-x-2 text-xs text-green-600 dark:text-green-400 font-bold">
                                                ...
                                                <span>{t.team.efficiency}: {Math.round((stats.done / (stats.total || 1)) * 100)}%</span>
                                            </div>
                                            <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase">
                                                {stats.done} {t.team.completed}
                                            </span>
                                        </div>
                                    </div>
                                </motion.div>
            ...
            <AnimatePresence>
                {showInviteModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
                        ...
                        >
                            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t.team.inviteModalTitle}</h2>
                            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">{t.team.inviteModalDesc}</p>

                            {!invitationToken ? (
                                <form onSubmit={handleInvite} className="space-y-4">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t.team.emailLabel}</label>
                                        <input
                                            type="email"
                                            required
                                            value={inviteEmail}
                                    ...
                                    >
                                        {isInviting ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : t.team.generateToken}
                                    </button>
                                </form>
                            ) : (
                                <div className="space-y-6">
                                    <div className="rounded-xl bg-green-50 dark:bg-green-950/20 p-4 border border-green-100 dark:border-green-900/30">
                                        <p className="text-xs font-semibold text-green-700 dark:text-green-400">{t.team.tokenGenerated}</p>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t.team.shareToken}</label>
